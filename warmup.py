...
class AdamWarmup(Optimizer):
    ...
    def _resource_apply_dense(self, grad, var):
        ...
        lr_t = tf.where(
            ...
            min_lr + (lr_t - min_lr) * (1.0 - tf.minimum(local_step-warmup_steps, decay_steps-warmup_steps) / (decay_steps-warmup_steps)),
        ...
        if self._initial_weight_decay > 0.0:
            if self._weight_decay_pattern is None:
                weight_decay = self._get_hyper('weight_decay', var_dtype)
                var_update += weight_decay * var
            else:
                for pattern in self._weight_decay_pattern:
